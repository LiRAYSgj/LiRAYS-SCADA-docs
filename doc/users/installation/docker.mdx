# Docker Deployment

This guide describes a container-based deployment for LiRAYS-SCADA.

## Pull Image

```sh
docker pull ghcr.io/liraysgj/lirays-scada:latest
```

## Quick Start (`docker run`)

```sh
docker run -d \
  --name lirays-scada \
  --restart unless-stopped \
  -p 8245:8245 \
  -v lirays-data:/data \
  ghcr.io/liraysgj/lirays-scada:latest
```

Access URL:

- `http://127.0.0.1:8245`

## Compose Deployment (`compose.yaml`)

```yaml
services:
  lirays-scada:
    image: ghcr.io/liraysgj/lirays-scada:latest
    container_name: lirays-scada
    restart: unless-stopped
    ports:
      - "8245:8245"
    volumes:
      - lirays-data:/data
      # Optional custom config:
      # - ./settings.yaml:/app/settings.yaml:ro
    # command: ["--config", "/app/settings.yaml"]

volumes:
  lirays-data:
```

Start and stop:

```sh
docker compose up -d
docker compose down
```

## Custom Configuration

### Option 1: Mount `settings.yaml`

```sh
docker run -d \
  --name lirays-scada \
  --restart unless-stopped \
  -p 8245:8245 \
  -v lirays-data:/data \
  -v "$(pwd)/settings.yaml:/app/settings.yaml:ro" \
  ghcr.io/liraysgj/lirays-scada:latest \
  --config /app/settings.yaml
```

### Option 2: Use Environment Variables

```sh
docker run -d \
  --name lirays-scada \
  --restart unless-stopped \
  -p 8245:8245 \
  -v lirays-data:/data \
  -e AUTH_ENABLED=true \
  -e AUTH_SECRET="replace_with_strong_secret" \
  -e TLS_ENABLE=false \
  -e METRICS_REAL_TIME=true \
  ghcr.io/liraysgj/lirays-scada:latest
```

For complete variable and precedence details, see [Settings File Configuration](/users/configuration/settings_file.mdx).

## Verify

```sh
docker ps --filter name=lirays-scada
docker logs --tail=200 -f lirays-scada
docker inspect lirays-scada --format '{{.State.Status}}'
```

## Upgrade

`docker run` workflow:

```sh
docker pull ghcr.io/liraysgj/lirays-scada:<new-tag>
docker stop lirays-scada
docker rm lirays-scada
docker run -d \
  --name lirays-scada \
  --restart unless-stopped \
  -p 8245:8245 \
  -v lirays-data:/data \
  ghcr.io/liraysgj/lirays-scada:<new-tag>
```

Compose workflow:

```sh
docker compose pull
docker compose up -d
```

## Remove Deployment

Keep volume (preserve data):

```sh
docker stop lirays-scada
docker rm lirays-scada
```

Full cleanup (container, image, and volume):

```sh
docker stop lirays-scada || true
docker rm lirays-scada || true
docker rmi ghcr.io/liraysgj/lirays-scada:latest || true
docker volume rm lirays-data || true
```
